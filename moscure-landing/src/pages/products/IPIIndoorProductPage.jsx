      ...
      <div className="flex gap-2 mt-3 flex-wrap">
        {images.map((img, i) => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`relative w-14 h-14 rounded-lg border-2 overflow-hidden cursor-pointer transition-colors shrink-0 ${i === activeIndex
...
function VideoTestimonials() {
  const videoIds = ['SfP9ZsH0Sf4', 'BXlIOvM3LpE', 'GaSK2J-HMy4', 'ddaxKgiJzOY', 'uO3goxBOjOM']
...
export default function IPIIndoorProductPage({ onNavigate }) {
  ...
  // Auto-rotate product images every 3 seconds (pause if video is active)
  useEffect(() => {
  ...
  const handleWhatsApp = () => {
    window.open(
      `https://wa.me/?text=${encodeURIComponent('Check out the Moscure IPI Indoor Mosquito Trap — Chemical-Free, UV LED, ₹1,399\nhttps://www.moscure.com/products/moscure-ipi-indoor-mosquito-trap')}`,
      '_blank',
      'noopener,noreferrer'
    )
  }
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-12 py-6 pb-16 grid grid-cols-1 lg:grid-cols-2 gap-x-14 gap-y-10 items-start">
          ...
          >
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <motion.a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent('Moscure IPI — Chemical-Free Indoor Mosquito Trap. UV LED, 400 sq ft coverage. ₹1,399')}&url=${encodeURIComponent('https://moscure.in/products/moscure-ipi-indoor-mosquito-trap')}`}
                target="_blank"
